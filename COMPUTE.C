...
void	CompNormal( int far *p1, int far *p2, int far *p3, int far *normal)
...
void	CompVertices( s_object *obj)
...
static	void near	CompCoordinates( s_object *obj)
...
static	uchar near	ClipProjectPoly( uchar		n,
					 uchar		visible,
					 s_object	*obj)
...
{	extern	 char	ExtendedClip, CheckOffRoad, OnObject;
	...
	*v2++ = auxbuf[2];
	...
	{	/* SAVE SURFACE VECTORS FOR USE IN SIMULATION MODEL. */
		...
		ObjFace[0] = ClipVert[0];	ObjFace[1] = ClipVert[1];	ObjFace[2] = ClipVert[2];
		ObjFace[3] = ClipVert[3];	ObjFace[4] = ClipVert[4];	ObjFace[5] = ClipVert[5];
		ObjFace[6] = ClipVert[6];	ObjFace[7] = ClipVert[7];	ObjFace[8] = ClipVert[8];
	}
...
static	void near	ClipCoordinates( s_object *obj)
...
static	int * near	ClipStreet( int *vert,		/* pointer to 3D coordinates */
