	...
	if (!(key2 & 0x7F))
	{
		...
		{       if (isupper(key))
			...
			{	/* STANDARD KEYBOARD COMMANDOS */
				...
				case ESC  :	GameMode = FALSE;
						SystemExit( "");
						break;
				#if (1)

					case 'c'  :	GameMode = FALSE;
							QualifyFlag = TRUE;
							break;

					case 'q'  :	GameMode = FALSE;
							QualifyFlag = FALSE;
							break;
				#endif
			}
			...
			{	switch (key)
				{	case '1' :	car.ShiftFlag = 1;
							break;
					case '2' :	car.ShiftFlag = 2;
							break;
					case '3' :	car.ShiftFlag = 3;
							break;
					case '4' :	car.ShiftFlag = 4;
							break;
					case 'n' :	car.ShiftFlag = 5;
							break;
				}
			}
		}
...
{	extern	char	MapName[], SelectName[], LoadName[], PanelName[];
	...
	filesOK &= FixedRead( MapName,    Tim, WindowSize);	Uncomp( MapScreen);
	filesOK &= FixedRead( SelectName, Tim, WindowSize);	Uncomp( SelectScreen);
	filesOK &= FixedRead( PanelName,  Tim, ScreenSize);	Uncomp( AuxScreen);
...
void	Insert( int distance, s_object *obj)
...
{       extern	 char		SpinFlag;
	...
	fr->x = car.mx;		fr->yaw   = car.yaw;
	fr->y = car.my;		fr->pitch = car.pitch;
	fr->z = car.mz;		fr->roll  = car.roll;
...
{	extern	 char		RedSample, StartFlag, TimeOutFlag;
	...
	f1 = RedPointer;	h1 = SamplePeriod - RedSample;
	f2 = f1 + 1;		h2 = RedSample;
...
{	extern	 char		BlueSample, StartFlag, TimeOutFlag;
	...
	f1 = BluePointer;	h1 = SamplePeriod - BlueSample;
	f2 = f1 + 1;		h2 = BlueSample;
	...
	if (!TimeOutFlag)
	{	if (   (abs(bc->worldX - FinishX) < 200)
		    && (abs(bc->worldZ - FinishZ) < 200)
		    && (abs(bc->worldY - FinishY) <  40)
		   )
		{	TimeFlag = FALSE;
			TimeOutFlag = TRUE;
			LapFinished = TRUE;
...
void	TimeString( char *p, long time)
...
void	SetCar( int segment, int lane, int row, int side)
...
uchar	SetObjClass( s_object *obj)
...
s_object *	SearchObject( int type)
...
void	CopyCarPosition( s_object *obj)
...
{	extern	char	BonusFlag, MessageTime, MessageCode;
	...
	switch (Difficulty)
	{	case 1 :	SubBCD( 0x0003L, &bonus);
				break;
		case 2 :	SubBCD( 0x0005L, &bonus);
				break;
		case 3 :	SubBCD( 0x0007L, &bonus);
				break;
	}
