...
static	void	DrawBackground( int time)
...
{	extern	 int	coordinate[], PolyVertex[];
	...
	for (i = 0; i < 40; i++)
	{	PolyVertex[0] = *p++;	PolyVertex[1] = *p++;
		PolyVertex[2] = *p++;	PolyVertex[3] = *p++;
		PolyVertex[4] = *p++;	PolyVertex[5] = *p++;
		PolyVertex[6] = *p++;	PolyVertex[7] = *p++;
...
static	void	DisplayFlags( int time)
...
static	int	DrawArrowAndBox( void)
...
static	int	PrintSelectScreen( void)
...
{	extern	 char	SteerSelect, GearSelect, MouseFlag;
	...
	if (key == 1)	if (--ypos < 1)	ypos = 6;
	if (key == 2)	if (++ypos > 6)	ypos = 1;
	if (key == 3)	if (--xpos < 1)	xpos = 4;
	if (key == 4)	if (++xpos > 4)	xpos = 1;
