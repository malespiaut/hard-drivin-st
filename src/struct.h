...
#define TrackName "data/track.dat"
#define DataBaseName "data/objects.dat"
#define SoundName "data/snd.pin"
#define ScoreName "data/list"
...
typedef struct so
{
  ...
  unsigned int radius;  // object's radius.
  unsigned int worldX;  // position in world coordinate system.
  int worldY;
  unsigned int worldZ;
  int pitch, yaw, roll;       // object's angles.
  unsigned int vertices;      // number of vectors.
  unsigned char normals;      // number of normal vectors.
  unsigned char polygons;     // number polygons.
  unsigned char small, large; // models for 'small' and 'large' version.
  unsigned char pre_rotated;  // TRUE, if object model is prerotated.
  unsigned char type;         // object model in use.
  unsigned char class;        // object's class.
  unsigned char visible;      // visibility flag.
  unsigned char active;       // TRUE, if object in use
  unsigned char message;
  unsigned char collision; // TRUE, if viewpoint inside the object
  unsigned char color;     // individual color, used for cars
  unsigned char clip3D;    // TRUE, part of the object is behind viewpoint.
  ...
  unsigned char* faces;     // pointer to polygon description list.
} s_object;
...
typedef struct
{
  unsigned int x;
  int y;
  unsigned int z; // CURRENT VIEWPOINT
  unsigned int mx;
  int my;
  unsigned int mz; // MIDPOINT, SAME LEVEL AS WHEELS
  unsigned int newX;
  int newY;
  unsigned int newZ; // MIDPOINT, USED NEXT FRAME
...
typedef struct
{
  ...
  unsigned char objNum;    // NUMBER OF USED OBJECT.
  unsigned char TrackUsed;
  unsigned char StartTrack;
  unsigned char HomeTrack;
  int position;               // POSITION ON SEGMENT.
  unsigned char lane;         // LANES: 0 = SLOW,BORDER, 1 = FAST,MIDDLE
  char OnStreet;              // TRUE, IF CAR REACHES A NEW SEGMENT.
  char direction;             // 1 = IN TRACK DIRECTION, -1 = AGAINST TRACK DIRECTION
  unsigned char acceleration; // CAR'S MAXIMAL ACCELERATION.
  char maxspeed;              // NOMINAL SPEED.
  char speed;                 // VEHICLE'S SPEED.
  unsigned char color;        // VEHICLE'S COLOR.
