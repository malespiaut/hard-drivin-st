...
static	void near	TurnPitch( int deltaPitch)
...
static	void near	TurnYaw( int deltaYaw)
...
{	extern	 s_car	car;
	...
	if (cosP < 0)	deltaYaw = -3*deltaYaw >> 1;
...
{	extern	 s_car	car;
	...
	h = - car.newYaw;	sinY =   sinus(h);
				cosY = cosinus(h);
	h = - car.newPitch;	sinP =   sinus(h);
				cosP = cosinus(h);
	h = - car.newRoll;	sinR =   sinus(h);
				cosR = cosinus(h);
	...
	h = NormLength;	 CarNormX = rotate( h, -sinR);
			 CarNormY = rotate( h,  cosR);

	h = CarNormY;	 CarNormY = rotate( h,  cosP);
			 CarNormZ = rotate( h,  sinP);

	h = CarNormX;	 CarNormX = rot1( h, CarNormZ, sinY, cosY);
...
{	extern	 char	SpinFlag, SkidFlag, AccidentFlag, CarInLoop;
	...
	if (!AccidentFlag)
	{
		...
		car.newPitch = car.pitch;  TurnPitch( car.deltaPitch);
	...
	/* NORMALIZE ANGLES */
		...
		if (car.newPitch >  4*180)	car.newPitch -= 4*360;
	else	if (car.newPitch < -4*180)	car.newPitch += 4*360;

		if (car.newRoll >   4*180)	car.newRoll  -= 4*360;
	else	if (car.newRoll <  -4*180)	car.newRoll  += 4*360;


	/* COMPUTE TEMPORARY TRIG. VALUES FOR SIMULATION MODEL. */

	sinY = sinus(car.newYaw);	cosY = cosinus(car.newYaw);
	sinP = sinus(car.newPitch);	cosP = cosinus(car.newPitch);
	sinR = sinus(car.newRoll);	cosR = cosinus(car.newRoll);
}
...
{	extern	 char		CheckOffRoad, SpinFlag, SkidFlag;
	...
	if ((car.gear == 0) || (!car.WheelFlags))
	{
		...
		deltaRPM = newRPM - car.rpm;
			...
			if (deltaRPM >  MaxDeltaRPM)	deltaRPM =  MaxDeltaRPM;
		else	if (deltaRPM < -MaxDeltaRPM)	deltaRPM = -MaxDeltaRPM;
	...
	{	if (SpinFlag || (abs(car.wheelSpeed - car.speed) > 10))
		...
		h -= car.rpm;
			...
			if (h >  MaxDeltaRPM)	h =  MaxDeltaRPM;
		else	if (h < -MaxDeltaRPM)	h = -MaxDeltaRPM;
...
static	int near	CompDistance( int x, int y, int z, char set1)
...
{	char	flag;
	...
	switch (set1)
	{	case 3 :	s1x1 = -dx12;	s1y1 = -dy12;	s1z1 = -dz12;
				s1x2 = -dx14;	s1y2 = -dy14;	s1z2 = -dz14;
				s2x1 = -dx22;	s2y1 = -dy22;	s2z1 = -dz22;
				s2x2 = -dx23;	s2y2 = -dy23;	s2z2 = -dz23;
				break;

		case 2 :	s1x1 = -dx11;	s1y1 = -dy11;	s1z1 = -dz11;
				s1x2 = -dx13;	s1y2 = -dy13;	s1z2 = -dz13;
				s2x1 = -dx21;	s2y1 = -dy21;	s2z1 = -dz21;
				s2x2 = -dx24;	s2y2 = -dy24;	s2z2 = -dz24;
				break;

		case 1 :	s1x1 = dx11;	s1y1 = dy11;	s1z1 = dz11;
				s1x2 = dx12;	s1y2 = dy12;	s1z2 = dz12;
				s2x1 = dx21;	s2y1 = dy21;	s2z1 = dz21;
				s2x2 = dx22;	s2y2 = dy22;	s2z2 = dz22;
				break;

		case 0 :	s1x1 = dx13;	s1y1 = dy13;	s1z1 = dz13;
				s1x2 = dx14;	s1y2 = dy14;	s1z2 = dz14;
				s2x1 = dx23;	s2y1 = dy23;	s2z1 = dz23;
				s2x2 = dx24;	s2y2 = dy24;	s2z2 = dz24;
				break;
	}
...
static	int near	CompHeight( int x, int y, int z)
...
{	extern	int	ObjFace[];
	...
	dx1 = ObjFace[3] - x1;		dx2 = ObjFace[6] - x1;
	dy1 = ObjFace[4] - y1;		dy2 = ObjFace[7] - y1;
	dz1 = ObjFace[5] - z1;		dz2 = ObjFace[8] - z1;
...
{	int	k[4*3];
	...
	k[0] = x1;	k[ 1] = y1;	k[ 2] = z1;
	k[3] = x2;	k[ 4] = y2;	k[ 5] = z2;
	k[6] = x3;	k[ 7] = y3;	k[ 8] = z3;
	k[9] = x4;	k[10] = y4;	k[11] = z4;
