...
{	extern	int		PolyVertex[];
	...
	{	BlankScreen();
		...
		PolyVertex[0] = -30;	PolyVertex[1] = -40;
		PolyVertex[2] = -50;	PolyVertex[3] =   0;
		PolyVertex[4] =  10;	PolyVertex[5] =  30;
		PolyVertex[6] =  50;	PolyVertex[7] =  10;
		Polygon(4);
	}

	CPUspeed = frames;

		if (CPUspeed < 12)	TuneSpeed = 2;	/* 32 MHZ */
	else	if (CPUspeed < 19)	TuneSpeed = 1;	/* 16 MHZ */
	else	if (CPUspeed < 30)	TuneSpeed = 0;	/*  8 MHZ */
	else				TuneSpeed = 3;	/*  5 MHZ */
}
...
static	void	CreditsScreen( void)
...
static	void	SelectGraphMode( void)
...
static	void huge *  GetBuffer( long size)
...
void	CopySmallScreen( int y1, int y2, void far *source, void far *dest)
...
static	long	FileLength( char *name)
...
static	int	DOS_create( char *name)
...
static	void	DOS_read( int handle, void far *addr, uint size)
...
static	void	DOS_write( int handle, void far *addr, uint size)
...
int	FileRead( char *name, void far *addr)
...
int	FixedRead( char *name, void far *addr, long size)
...
void	FileWrite( char *name, void far *addr, long size)
...
int	ReadBlockFile( s_BitImage *bi, char *name)
...
int	ReadShapeFile( s_BitImage *shape, char *FileName)
...
int	SetNewColor( int no, int col)
...
void far * GetPieceOfChunk( long amount)
...
static	void	SetBlock( int x1, int y1, int x2, int y2,
			  int both, s_block *bl)
...
{	extern	int	ScoreX, ScoreY,TimeX, TimeY;
	...
	MinX  = -160;		MaxX  = 159;
	MinY1 = -OrgY1;		MaxY1 = (WindowY-1)/2;
	MinY2 = -PanelY/2;	MaxY2 = (PanelY-1)/2;

	ScreenX = 320;
	ScreenY = 200;

	mphX = 188 - OrgX;	mphY = 171 - OrgY2;
	rpmX = 129 - OrgX;	rpmY = 171 - OrgY2;
	ampX =  88 - OrgX;	ampY = 159 - OrgY2;
	tmpX =  88 - OrgX;	tmpY = 186 - OrgY2;
	oilX = 230 - OrgX;	oilY = 159 - OrgY2;
	gasX = 230 - OrgX;	gasY = 186 - OrgY2;
	...
	ScoreY   = 156;		ScoreX   = 50;
	TimeY    = 172;		TimeX    = 50;
	BestY    = 182;		BestX    = 60;
	LastY    = 188;		LastX    = 60;
	SecondsY = 123;		SecondsX = 18;
	OffRoadY =  15;		OffRoadX = 14;
				ListX	 = 11;
...
{	extern	char	MouseFlag, GraphMode, SteerSelect, GearSelect;
	...
	switch (GraphMode)
	{	case 1:
		case 2:		strcat( PanelName,  "CPX");
				strcat( MapName,    "CPX");
				strcat( SelectName, "CPX");
				strcat( LoadName,   "CPX");
				break;
		case 0:
		case 3:		strcat( PanelName,  "EPX");
				strcat( MapName,    "EPX");
				strcat( SelectName, "EPX");
				strcat( LoadName,   "EPX");
				break;

		case 4:         strcat( PanelName,  "VPX");
				strcat( MapName,    "VPX");
