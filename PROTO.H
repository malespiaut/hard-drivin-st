...
void	Convert( void far *addr, int x, int y);	/* Convert picture with given width and height. */
void	Convert2( void far *addr);		/* Convert shape. */
void	Convert3( void far *addr);		/* Convert block. */
void	Uncomp( void far *addr);		/* Uncompress picture. */
...
void	movebytes( void far *s, void far *d, int num);	/* Move count bytes from source to destination. */
void	movewords( void far *s, void far *d, int num);	/* Move count words from source to destination. */
...
int *	RotXY( int count, int cos, int sin,
	       int far *dest, int far *source);
int *	RotXZ( int count, int cos, int sin,	/* Rotation of a [x,y,z] array with count elements. */
	       int far *dest, int far *source);
int *	RotYZ( int count, int cos, int sin,
	       int far *dest, int far *source);
...
void	PrintDigit( char digit, int x,
		    int offset);		/* Print single digit at {x,y}. */
void	PrintString( int x, int y,
		     char *string);		/* Print string of large characters. */
						/* STRINGS HAVE TO BE TERMINATED WITH '.' */
void	TypeString(  int x, int y,
		     char *string);		/* Print string of small characters. */
						/* STRINGS HAVE TO BE TERMINATED WITH '#' */
...
void	SysToVid( int x1, int y1,
		  int x2, int y2,
		  void far *source,
		  void far *dest);		/* NO CLIPPING AND RANGE CHECK !!. */
void	VidToVid( int x1, int y1,
		  int x2, int y2,
		  void far *source,
		  void far *dest);		/* NO CLIPPING AND RANGE CHECK !!. */
void	CopyBlock( int sx1, int sy1, int sx2, int sy2,
		   int dx1, int dy1,		/* BitBlock copy routine. */
		   void far *source,
		   void far *dest);		/* NO CLIPPING AND RANGE CHECK !!. */
...
void	BlendIn( int count, int far *source);	/* Blend new picture into visible screen */
...
void	SetScreen( void far *screenaddr);	/* screenaddr points to the new screen RAM area. */
...
void	CopySmallScreen( int y1, int y2,
			 void far *source,
			 void far *dest);	/* Copy window screen. */
int	FileRead( char *name, void far *addr);	/* Read the file "name" into the buffer at addr. */
int	FixedRead( char *name, void far *addr,
		   long	size);			/* Read size bytes of "name" into the buffer at addr. */
void	FileWrite( char *name, void far *addr,
		   long size);			/* Read the file "name" into the buffer at addr. */
...
void far *GetPieceOfChunk( long size);		/* Allocate one buffer of given size. */
...
void	DrawBitBlock( int x, int y,
		      s_BitImage shape);	/* Draw image at given position. */
void	DrawShape( int x, int y,
		   s_BitImage shape);		/* Draw shape at a given position. */
...
void	SetCar( int segment, int lane,
		int row,     int side);		/* Set car to that position. */
uchar	SetObjClass( s_object *obj);		/* Set attributes of object class n. */
s_object * SearchObject( int type);		/* Find first active object of given type. */
void	SetArrows( uchar status);		/* Enable/Disable gridarrows. */
...
uint	Sqrt( ulong n);				/* Compute square root. */
void	CompNormal( int far *p1,
		    int far *p2,
		    int far *p3,
		    int far *normal);		/* Compute normal vector out of given three coordinates. */
