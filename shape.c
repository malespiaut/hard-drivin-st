...
static	void	Shift( int far *source,
		       int far *dest,
		       int shift,
		       s_BitImage *shape)
...
void	PreShift( s_BitImage *shape, int far *buf, int far *dest)
...
void	DrawBitBlock( register int x, int y, s_BitImage *shape)
...
{	extern	 int	MinY, MaxY, MinX, MaxX;
	...
	register long	mask;
		 ...
		 int	h, dx, dy;
	static	 long	bitmask[] = {	0x00000000L,
					0xC000C000L,
					0xF000F000L,
					0xFC00FC00L,
					0xFF00FF00L,
	...
	if ((x > MaxX) || (x+shape->width < MinX))	return;
	if (y-shape->height > MaxY)			return;
...
void	DrawShape( int x, int y, s_BitImage *shape)
