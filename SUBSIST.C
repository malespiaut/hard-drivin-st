...
substitute()
{
   ...
   if(!state4){   /* if substitution is called for the first time */
		flush_mobs();
		process_mobs();
      clear_pf1();
      clear_pf2();
   ...
   else{          /* if substitution is called for the nth time */
      switch(temp[15]){
...
batter_substitution_init()
{
   ...
   for(k=9;k<15;k++) {
	   pal = 2+teamup;
      if (BATTERS[teamup][lineup[teamup][k]].playing == 0xFF)  pal = 1;
      print_message_pal(22,k+7,BATTERS[teamup][lineup[teamup][k]].name,pal);   
      message_buffer[0] = (BATTERS[teamup][lineup[teamup][k]].misc & 0x80) ? 'S' :
          (BATTERS[teamup][lineup[teamup][k]].misc & 0x01) ? 'L' : 'R';   
...
full_stats()
{
   ...
   i = BATTERS[temp[6]][number].hr;
	if ((message_buffer[0] = i / 10 + 48) == 48) message_buffer[0] = ' ';
	message_buffer[1] = (i % 10) + 48;
	message_buffer[2] = '\0';
	print_message_pal(x,6, message_buffer,pal);
	if (number < 9) pass_vars[0] = BATTERS[temp[6]][number].playing;
	else pass_vars[0] = BATTERS[temp[6]][number].position;
	pass_vars[1] = number;
	get_pposition();
	...
	message_buffer[0] = (BATTERS[temp[6]][number].misc & 0x80) ? 'S' :
       (BATTERS[temp[6]][number].misc & 0x01) ? 'L' : 'R';   
	message_buffer[1] = '\0';
...
batter_substitution_move()
{
	...
	if (Offense_CTRL_edge & (JS_START)) state4 = 0xFF;   /* RETURN */
...
fielder_substitution_init()
{
   ...
   for(k=0;k<9;k++) {
      if (lineup[temp[6]][k] < 0x80){
      	...
      	message_buffer[0] = (BATTERS[temp[6]][lineup[temp[6]][k]].misc & 0x80) ? 'S' :
         	 (BATTERS[temp[6]][lineup[temp[6]][k]].misc & 0x01) ? 'L' : 'R';   
   ...
   for(;k<15;k++) {
	   pal = 2+teamup^1;
      if (BATTERS[temp[6]][lineup[temp[6]][k]].playing == 0xFF) pal = 1;
...
fielder_substitution_move()
{
	...
	if (Defense_CTRL_edge & (JS_START)) state4 = 0xFF;   /* RETURN */
}
...
pitcher_substitution_init()
{
   ...
   for(k=0;k<10;k++) {
	   pal = 2+temp[6];
		if (PITCHERS[temp[6]][k].available)  pal = 1;
		else if (temp[7] == 0xFF) temp[7] = k;
