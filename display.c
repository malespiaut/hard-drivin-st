...
static	void near	PrintMountains( int y)
...
{       extern	 char	GraphMode;
	...
	if (z > 0)
	{	if (abs(car.roll) < 8)
		{	SolidColor(blue);	WideBox(MinY,y);
			SolidColor(green);	WideBox(y+1,MaxY);
		...
		{		if ( 4*30 < car.pitch)
				...
				{	int	x, dx, dy;
					...
					PolyVertex[0] = x + dx;		PolyVertex[1] = y + dy;
					PolyVertex[2] = x + dx - dy;	PolyVertex[3] = y + dy + dx;
					PolyVertex[4] = x - dx - dy;	PolyVertex[5] = y - dy + dx;
					PolyVertex[6] = x - dx;		PolyVertex[7] = y - dy;

					SolidColor(green);	Polygon(4);
					...
					SolidColor(blue);	Polygon(4);
				}
		}
	}
	else
	{	SolidColor(blue);	WideBox(y+1,MaxY);
		SolidColor(green);	WideBox(MinY,y);
	}
...
{	extern	 char		LapMode, QualifyFlag, DemoMode;
	...
	{		if (LapMode)		b = &MessageRect[2];
		else	if (QualifyFlag)	b = &MessageRect[1];
		else				b = &MessageRect[0];
	}
				CopyBlock( b->sx1, b->sy1, b->sx2, b->sy2, b->dx1, b->dy1, AuxScreen, Buffer2);

	b = (car.autoFlag) ? &AutoRect : &ManualRect;
				CopyBlock( b->sx1, b->sy1, b->sx2, b->sy2, b->dx1, b->dy1, AuxScreen, Buffer2);
	b = &ScoreRect;		CopyBlock( b->sx1, b->sy1, b->sx2, b->sy2, b->dx1, b->dy1, AuxScreen, Buffer2);
	b = &TimeRect;		CopyBlock( b->sx1, b->sy1, b->sx2, b->sy2, b->dx1, b->dy1, AuxScreen, Buffer2);
	b = &MidRect;		CopyBlock( b->sx1, b->sy1, b->sx2, b->sy2, b->dx1, b->dy1, AuxScreen, Buffer2);
	b = &BarRect;		CopyBlock( b->sx1, b->sy1, b->sx2, b->sy2, b->dx1, b->dy1, AuxScreen, Buffer2);

	/* HILITE THE CHOOSEN GEAR. */

	b = car.gearBlock;	CopyBlock( b->sx1, b->sy1, b->sx2, b->sy2, b->dx1, b->dy1, AuxScreen, Buffer2);
}
...
{	extern	 char	SloMoFlag, DemoMode;
	...
	PolyVertex[0] = rpmX+x;		PolyVertex[1] = rpmY-y;
	PolyVertex[2] = rpmX-y1;	PolyVertex[3] = rpmY-x1;
	PolyVertex[4] = rpmX+y1;	PolyVertex[5] = rpmY+x1;
	Polygon(3);
	...
	PolyVertex[0] = mphX+x;		PolyVertex[1] = mphY-y;
	PolyVertex[2] = mphX-y1;	PolyVertex[3] = mphY-x1;
	PolyVertex[4] = mphX+y1;	PolyVertex[5] = mphY+x1;
	Polygon(3);
	...
	{	/* DRAW HORIZONTAL STEERING INDICATOR. */
		...
		NewColor(blue);
		x = (SteerX - 320) / 30 - 2;
		MoveTo( x, -22);	DrawTo( x, -20);	x++;
		MoveTo( x, -22);	DrawTo( x, -20);
		...
		MoveTo( -1, x);		DrawTo( -2, x);		x++;
		MoveTo( -1, x);		DrawTo( -2, x);
...
static	void near	ShowAllCarsInTheLoop( s_object *loop)
...
{	extern	 int		VisibleObjects;
	...
	{	obj = (pr++)->obj;
		...
		{	if (obj->visible && obj->worldY >= 0)
			{	if (   abs(obj->worldX - x) < radius
				    && abs(obj->worldZ - z) < radius)
				{	PrintCar(obj);
					obj->visible = FALSE;
				}
			}
...
void	PrintUnderpass( s_object *obj)
...
static	void near	PrintHillRoads( int type)
	...
	*/
	...
	{	obj = (p++)->obj;
		...
		{	(obj->DisplayProc)(obj);
			...
			if (obj->class == c_hill)
			{	switch (obj->large)
				{	case  8 :   PrintHillRoads( f_hill1);
						    break;

					case  9 :   PrintHillRoads( f_hill2);
						    break;

					case 48 :   PrintHillRoads( f_hill4);
						    break;

					case 61 :   PrintHillRoads( f_hill3);
						    break;
				}
			}
		}
